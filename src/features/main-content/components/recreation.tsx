...
const RecreationItem = ({ category }: { category: any }) => {
  ...
  return (
    <div
      className={`relative bg-[#141B3440] rounded-xl overflow-hidden group cursor-pointer group ${category.size}`}
    >
      {category.image ? (
        <img
          src={category.image}
      ...
      <h3
        className={`absolute top-4 left-4 text-lg font-semibold ${category.color}`}
      >
        {category.title}
      </h3>
    </div>
