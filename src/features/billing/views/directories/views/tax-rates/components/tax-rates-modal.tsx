...
import {
  ...
  getTaxRate,
  updateTaxRate,
} from '../../../api/getTaxRates'
import MoneyOnHand from '@/components/icons/money-on-hand'
...
const TaxRatesModal = ({ refetch }: UserModalProps) => {
  const { t } = useTranslation()
  const [searchParams] = useSearchParams()
  const navigate = useNavigate()
  const { pathname } = useLocation()
  const { isModalOpen, closeModal } = useTaxRatesModalStore(state => state)
  ...
  const { data, refetch: fetching } = useQuery({
    ...
    queryFn: () => getTaxRate({ id: editUserId }),
    enabled: !!editUserId,
  ...
  const handleUserSave = useMutation({
    mutationFn: (values: any) => {
      ...
      if (editUserId) {
        return updateTaxRate({ id: editUserId, queryParams: formattedValues })
      }

      return createTaxRate(formattedValues)
    },
    onSuccess: res => {
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <Form.Item label={t('Фискальный флаг')} name="status" rules={[]}>
              <Switch
                className="select-shadow"
